...
import generateDownloadAccountLink, {
  __RewireAPI__ as linkRewireAPI
...
describe("modules/auth/helpers/generate-download-account-link.js", () => {
  ...
  test({
    ...
    assertions: () => {
      ...
      sinon
        .stub(Speedomatic, "byteArrayToHexString")
        .callsFake(privateKey => privateKey);
      ...
      sinon
        .stub(keythereum, "generateKeystoreFilename")
        .callsFake(address => address);
      ...
      assert(
        keythereum.generateKeystoreFilename.calledOnce,
        `didn't call 'keythereum.generateKeystoreFilename' once as expected`
      );
    }
  });
});
