...
describe("modules/auth/actions/transfer-funds.js", () => {
  ...
  test({
    ...
    assertions: (done, store) => {
      ...
      assert(
        ...
        `didn't call 'console.error' once as expected`
      );
  ...
  test({
    ...
    assertions: (done, store) => {
      ...
      store.dispatch(transferFunds(10, ETH, "0xtest2"));
      assert(
        addNotification.calledOnce,
        `didn't call 'addNotifications' once as expected`
      );
      assert(
        updateAssets.calledOnce,
        `didn't call 'updateAssets' once as expected`
      );
      done();
    }
  });
