...
describe("modules/markets/actions/load-unclaimed-fees.js", () => {
  ...
  describe("loadUnclaimedFees", () => {
    const {
      loadUnclaimedFees,
      __RewireAPI__
    } = require("modules/markets/actions/load-unclaimed-fees");
    const ACTIONS = {
      ...
      augurNode: {
        submitRequest: (label, p, cb) => {
          assert.deepEqual(
            ...
            `Didn't call the correct getter function from augurNode`
          );
          if (!p.marketIDs)
            return cb({ error: "must include marketIDs parameter" });
    ...
    test({
      ...
      assertions: store => {
        ...
        store.dispatch(
          loadUnclaimedFees(undefined, (err, unclaimedFees) => {
            assert.isNull(err, `Didn't return null for error as expected`);
            assert.deepEqual(
              unclaimedFees,
              {},
              `Expected unclaimedFees to be an empty object`
            );
          })
        );
    ...
    test({
      ...
      assertions: store => {
        // force an error
        store.dispatch(
          loadUnclaimedFees(null, (err, unclaimedFees) => {
            const expectedError = { error: "must include marketIDs parameter" };
            assert.deepEqual(
              err,
              expectedError,
              `Didn't return the expected error`
            );
            assert.isUndefined(
              unclaimedFees,
              `Expected unclaimedFees to be an empty object`
        ...
        store.dispatch(
          loadUnclaimedFees(
            ...
            (err, unclaimedFees) => {
              ...
              assert.deepEqual(
                unclaimedFees,
                expectedUnclaimedFees,
                `Unexpected unclaimed Fees Object`
              );
            }
          )
        );
