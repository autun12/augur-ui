...
describe("modules/markets/actions/load-markets-by-category.js", () => {
  ...
  const test = t => {
    it(t.description, () => {
      ...
      if (t.toTest === "err")
        AugurJS.augur.markets.getMarketsInCategory.yields(
          "failed with err",
          null
        );
      if (t.toTest === "null-markets")
        AugurJS.augur.markets.getMarketsInCategory.yields(null, null);
      if (t.toTest === "array")
        AugurJS.augur.markets.getMarketsInCategory.yields(null, ["0x1, 0x2"]);
      if (t.toTest === "empty-array")
        AugurJS.augur.markets.getMarketsInCategory.yields(null, []);
      const action = proxyquire(
        "../../../src/modules/markets/actions/load-markets-by-category",
        {
  ...
  test({
    description:
      "should dispatch the expected actions with no error + no marketIDs returned",
    toTest: "null-markets",
    assertions: actions => {
      const expected = [
        {
  ...
  test({
    description:
      "should dispatch the expected actions with no error + array of returned marketIDs",
    toTest: "array",
  ...
  test({
    description:
      "should dispatch the expected actions with no error + an empty array of marketIDs",
    toTest: "empty-array",
    assertions: (actions, loadMarketsInfo) => {
      const expected = [
        {
