...
describe("modules/portfolio/selectors/nav-items", () => {
  ...
  const stubbedMyPositionsSummary = sinon
    .stub(selectors, "selectMyPositionsSummary")
    .callsFake(() => ({
      numPositions: formatNumber(10, { denomination: "positions" }),
      totalNet: formatEtherTokens(2)
    }));
  const stubbedMyMarketsSummary = sinon
    .stub(selectors, "selectMyMarketsSummary")
    .callsFake(() => ({
      numMarkets: 30,
      totalValue: 10
    }));
  const stubbedMyReportsSummary = sinon
    .stub(selectors, "selectMyReportsSummary")
    .callsFake(() => ({
      numReports: 10,
      netRep: 5
    }));
  ...
  it(`should call 'selectMyPositionsSummary' once`, () => {
    assert(
      stubbedMyPositionsSummary.calledOnce,
      `Didn't call 'selectMyPositionsSummary' once as expected`
    );
  });
