...
describe(`modules/reporting/actions/submit-initial-report.js`, () => {
  ...
  test(`should call callback and history`, () => {
    ...
    store.dispatch(
      submitInitialReport(false, "testMarketId", 0, false, history, callback)
    );
  ...
  test(`should call only callback`, () => {
    submitInitialReportReqireAPI.__Rewire__("augur", augurSuccess);
    store.dispatch(
      submitInitialReport(false, null, 0, false, history, callback)
    );
    assert(callback.calledOnce, `Didn't call 'callback' once as expected`);
  ...
  test(`should call only callback with empty market id and bad outcome`, () => {
    submitInitialReportReqireAPI.__Rewire__("augur", augurSuccess);
    store.dispatch(
      submitInitialReport(false, "", "blah", false, history, callback)
    );
    assert(callback.calledOnce, `Didn't call 'callback' once as expected`);
    assert(history.push.notCalled, `Did call 'history' not expected`);
  });
  ...
  test(
    `should only call callback with empty market id, bad outcome and is invalid`,
    () => {
      submitInitialReportReqireAPI.__Rewire__("augur", augurSuccess);
      store.dispatch(
        submitInitialReport(false, "", "blah", true, history, callback)
      );
      assert(callback.calledOnce, `Didn't call 'callback' once as expected`);
      assert(history.push.notCalled, `Did call 'history' not expected`);
    }
  ...
  test(
    `should call both callback and history with good data not invalid`,
    () => {
  ...
  test(`should call callback and history with good data and is invalid`, () => {
    submitInitialReportReqireAPI.__Rewire__("augur", augurFailed);
    store.dispatch(
      submitInitialReport(false, "testMarketId", 0, true, history, callback)
    );
    assert(callback.calledOnce, `Didn't call 'callback' once as expected`);
    assert(history.push.calledOnce, `Did call 'history' not expected`);
  });
  ...
  test(
    `should call callback and not history with good data and is invalid`,
    () => {
      submitInitialReportReqireAPI.__Rewire__("augur", augurSuccess);
      store.dispatch(
        submitInitialReport(true, "testMarketId", 0, true, history, callback)
      );
      assert(callback.calledOnce, `Didn't call 'callback' once as expected`);
      assert(history.push.notCalled, `Did call 'history' not expected`);
    }
