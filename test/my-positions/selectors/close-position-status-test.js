...
describe("modules/my-positions/selectors/close-position-status", () => {
  ...
  const test = t => {
    test(t.description, () => {
      ...
      const result = selectClosePositionStatus({
        closePositionTradeGroups: t.state.closePositionTradeGroups,
        transactionsData: t.state.transactionsData
      });
  ...
  test({
    description:
      "should return CLOSE_DIALOG_CLi523OSING status if closePositionTradeGroups has a tradeGroupId and transactionsData does not house a corresponding tradeGroupId",
    state: {
