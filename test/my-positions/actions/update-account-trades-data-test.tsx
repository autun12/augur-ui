...
describe("modules/my-positions/actions/update-account-trades-data.js", () => {
  ...
  mockAddTradeTransactions.addTradeTransactions = sinon
    .stub()
    .returns({ type: MOCK_ACTION_TYPES.UPDATE_TRANSACTIONS_DATA });
  const mockloadAccountPositions = {
    loadAccountPositions: () => {}
  };
  mockloadAccountPositions.loadAccountPositions = sinon
    .stub()
    .returns({ type: "LOAD_ACCOUNT_POSITIONS" });
  const mockConvertTradeLogsToTransactions = {
    convertTradeLogsToTransactions: () => {}
  };
  sinon
    .stub(mockConvertTradeLogsToTransactions, "convertTradeLogsToTransactions")
    .callsFake((logType, data, marketID) => ({
  ...
  sinon
    .stub(mockUpdateOrders, "updateOrders")
    .callsFake((data, isAddition) => ({
  ...
  sinon
    .stub(mockLoadBidsAsksHistory, "loadAccountOrders")
    .callsFake(market => ({
    ...
    test({
      ...
      assertions: store => {
        const action = proxyquire(
          "../../../src/modules/my-positions/actions/update-account-trades-data",
          {
        ...
        store.dispatch(
          action.updateAccountBidsAsksData(
            { market: [{ "0xMARKETID": {} }] },
            "0xMARKETID"
          )
        );
        ...
        assert.deepEqual(
          actual,
          expected,
          `Didn't dispatch the expect action, getPositionInMarket`
        );
      }
    });
    test({
      ...
      assertions: store => {
        ...
        sinon
          .stub(mockAugur.augur.api.MarketFetcher, "getPositionInMarket")
          .callsFake(({ _account: account, _market: market }, callback) =>
            callback({})
          );
        const action = proxyquire(
          "../../../src/modules/my-positions/actions/update-account-trades-data",
        ...
        store.dispatch(
          action.updateAccountBidsAsksData({ "0xMARKETID": {} }, "0xMARKETID")
        );
        const actual = store.getActions();
        const expected = [
          {
        ...
        assert.deepEqual(
          actual,
          expected,
          `Didn't dispatch the expect action for getPositionInMarket`
        );
      }
    });
  });
  describe("updateAccountCancelsData", () => {
    test({
      ...
      assertions: store => {
        ...
        store.dispatch(
          action.updateAccountCancelsData({ "0xMARKETID": {} }, "0xMARKETID")
        );
        const actual = store.getActions();
        const expected = [
          {
