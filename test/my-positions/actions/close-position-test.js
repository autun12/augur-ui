...
describe("modules/my-positions/actions/close-position.js", () => {
  ...
  sinon
    .stub(mockUpdateTradesInProgress, "updateTradesInProgress")
    .callsFake(
      (marketId, outcomeId, side, numShares, limitPrice, maxCost, cb) => (
        dispatch,
        getState
      ) => cb()
    );
  const mockClearClosePositionOutcome = {
    clearClosePositionOutcome: sinon
      .stub()
      .returns({ type: MOCK_ACTION_TYPES.CLEAR_CLOSE_POSITION_OUTCOME })
  };
    ...
    const test = t =>
      test(t.description, () => {
        ...
        sinon
          ...
          .callsFake(
            (marketId, outcomeId, tradesInProgress, doNotCreateOrders, cb) => (
              dispatch,
              getState
            ) => {
