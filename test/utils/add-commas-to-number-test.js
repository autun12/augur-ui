...
describe("utils/add-commas-to-number.js", () => {
  ...
  test("should not insert commas with integers less than 999", () => {
    ...
    assert.strictEqual(
      addCommas(mockNumber),
      out,
      "returned value was not a string without commas"
    );
  });
  ...
  test("should insert commas with integers greater than 1,000", () => {
    ...
    assert.strictEqual(
      addCommas(mockNumber),
      out,
      "returned value was not a string with commas"
    );
  });
  ...
  test("should handle numbers with decimal places", () => {
    ...
    assert.strictEqual(
      addCommas(mockNumber),
      out,
      "returned value with decimal was not formatted correctly"
    );
  });
});
