  ...
  const reports = Object.keys(marketsWithAccountReport)
    .map(marketID => {
      const expirationDate =
        marketsWithAccountReport[marketID].expirationDate || null;
      const isFinal = marketsWithAccountReport[marketID].isFinal || null;
      const description =
        marketsWithAccountReport[marketID].description || null;
      const formattedDescription =
        marketsWithAccountReport[marketID].formattedDescription || null;
      const outcome = marketsWithAccountReport[marketID].marketOutcome || null;
      const outcomePercentage =
        (marketsWithAccountReport[marketID].proportionCorrect &&
          formatPercent(
            new BigNumber(
              marketsWithAccountReport[marketID].proportionCorrect,
              10
      ...
      const isReportEqual =
        (outcome != null && reported != null && outcome === reported) || null; // Can be done here
      const feesEarned = calculateFeesEarned(
        marketsWithAccountReport[marketID]
      );
      const repEarned =
        (marketsWithAccountReport[marketID].repEarned &&
          formatRep(marketsWithAccountReport[marketID].repEarned)) ||
        null;
      const endDate = (expirationDate && formatDate(expirationDate)) || null;
      const isChallenged =
        marketsWithAccountReport[marketID].isChallenged || null;
      const isChallengeable =
        isFinal != null && isChallenged != null && !isFinal && !isChallenged;
...
export const calculateFeesEarned = market => {
  if (!market.marketFees || !market.repBalance || !market.marketWeight)
    return null;
