...
export const selectClosePositionStatus = createSelector(
  ...
  (closePositionTradeGroups, transactionsData) => {
    const statuses = Object.keys(closePositionTradeGroups).reduce(
      (p, marketID) => {
        const outcomeStatuses = Object.keys(
          closePositionTradeGroups[marketID]
        ).reduce((p, outcomeID) => {
          const closePositionTransactionIDs = closePositionTradeGroups[
            marketID
          ][outcomeID].reduce((p, tradeGroupID) => {
            const transactionIDs = Object.keys(transactionsData).filter(
              transactionID =>
                transactionsData[transactionID].tradeGroupID === tradeGroupID
            );
          ...
          if (
            closePositionTradeGroups[marketID][outcomeID][0] ===
            CLOSE_DIALOG_FAILED
          ) {
          ...
          if (
            closePositionTransactionIDs.length === 0 &&
            closePositionTradeGroups[marketID][outcomeID]
          ) {
            return { ...p, [outcomeID]: CLOSE_DIALOG_CLOSING };
          }
          const numberOfFailedTransactions = closePositionTransactionIDs.filter(
            transactionID => transactionsData[transactionID].status === FAILED
          ).length;
          // Close Position Completely Failed
          if (
            numberOfFailedTransactions === closePositionTransactionIDs.length
          ) {
            delayClearTradeGroupIDs(marketID, outcomeID);
            return { ...p, [outcomeID]: CLOSE_DIALOG_FAILED };
          }
          // Close Position Completely Succeeded
          const numberOfSuccessfulTransactions = closePositionTransactionIDs.filter(
            transactionID => transactionsData[transactionID].status === SUCCESS
          ).length;
          if (
            numberOfSuccessfulTransactions ===
            closePositionTransactionIDs.length
          ) {
            delayClearTradeGroupIDs(marketID, outcomeID);
          ...
          if (
            ...
            numberOfFailedTransactions &&
            numberOfFailedTransactions + numberOfSuccessfulTransactions ===
              closePositionTransactionIDs.length
          ) {
            delayClearTradeGroupIDs(marketID, outcomeID);
            return { ...p, [outcomeID]: CLOSE_DIALOG_PARTIALLY_FAILED };
