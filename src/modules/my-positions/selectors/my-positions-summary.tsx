    ...
    markets.forEach(market => {
      market.outcomes.forEach(outcome => {
        ...
        qtyShares = qtyShares.plus(
          new BigNumber(outcome.position.qtyShares.value, 10)
        );
        totalRealizedNet = totalRealizedNet.plus(
          new BigNumber(outcome.position.realizedNet.value, 10)
        );
        totalUnrealizedNet = totalUnrealizedNet.plus(
          new BigNumber(outcome.position.unrealizedNet.value, 10)
        );
...
export const generatePositionsSummary = memoize(
  (numPositions, qtyShares, meanTradePrice, realizedNet, unrealizedNet) => {
    const totalNet = new BigNumber(realizedNet, 10).plus(
      new BigNumber(unrealizedNet, 10)
    );
    return {
