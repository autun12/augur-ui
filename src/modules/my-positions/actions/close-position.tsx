    ...
    dispatch(
      loadBidsAsks(marketID, () => {
        ...
        const positionOutcome = market
          ...
          : null;
        const positionShares = new BigNumber(
          getValue(positionOutcome, "position.qtyShares.value") || "0"
        );
        const orderBook = orderBooks[marketID];
        const userAddress = loginAccount && loginAccount.address;
        const bestFill = getBestFill(
          orderBook,
          positionShares.toNumber() > 0 ? BUY : SELL,
        ...
        if (bestFill.amountOfShares.equals(ZERO)) {
          ...
          dispatch(
            updateTradesInProgress(
              marketID,
              outcomeID,
              positionShares.toNumber() > 0 ? SELL : BUY,
              bestFill.amountOfShares.toNumber(),
...
export function getBestFill(
  ...
  Object.keys(orderBook[side] || {})
    .reduce((p, orderID) => {
      ...
      if (
        orderOutcome.outcome === outcomeID &&
        orderOutcome.owner !== userAddress
      ) {
        p.push(orderOutcome);
      }
      return p;
    ...
    .find(order => {
      amountOfShares = amountOfShares.plus(
        new BigNumber(order.fullPrecisionAmount)
      );
