...
const validateIsMarketValid = (validations, updateState, isMarketValid) => {
  ...
  if (isMarketValid) {
    ...
    updatedValidations.hasOwnProperty("selectedOutcome") &&
    updatedValidations.selectedOutcome === false
  ) {
...
const validateNumber = (
  ...
  switch (true) {
    ...
    case value > maxValue || value < minValue:
      updatedValidations[
        fieldName
      ] = `Please enter a ${humanName} between ${min} and ${max}.`;
      break;
    default:
...
const validateStake = (p, rawStake) => {
  ...
  const minStake =
    p.minimumStakeRequired instanceof BigNumber
      ? p.minimumStakeRequired
      : new BigNumber(p.minimumStakeRequired);
  const minStakeNum =
    p.minimumStakeRequired instanceof BigNumber
      ? p.minimumStakeRequired.toNumber()
      : p.minimumStakeRequired;
  let stake = rawStake;
...
type ReportingDisputeFormProps = {
  market: object,
...
> {
  static calcIsStakeRequired(p) {
    ...
    return [...p.market.otherOutcomes, currentOutcome].every(
      outcome => outcome.stake <= 0
    );
  }
  static calcMinimumStakeRequired(p) {
    ...
    if (ReportingDisputeForm.calcIsStakeRequired(p) && p.selectedOutcome) {
      minimumStake = "0";
      const outcomeObject = p.market.otherOutcomes.filter(
        outcome => outcome.name === p.selectedOutcome
      );
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.market.otherOutcomes !== nextProps.market.otherOutcomes) {
      this.props.updateState({
        stakeIsRequired: ReportingDisputeForm.calcIsStakeRequired(nextProps)
      });
    }
    if (
      this.props.selectedOutcome !== nextProps.selectedOutcome ||
      this.props.isMarketValid !== nextProps.isMarketValid
    ) {
      this.props.updateState({
        minimumStakeRequired: ReportingDisputeForm.calcMinimumStakeRequired(
          nextProps
        )
      });
    }
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevProps.minimumStakeRequired !== false &&
      prevProps.stake !== "" &&
      (prevProps.minimumStakeRequired !== this.props.minimumStakeRequired ||
        prevProps.isMarketValid !== this.props.isMarketValid)
    ) {
    ...
    return (
      ...
      >
        ...
        <li>
          ...
          <p className={FormStyles["text--field-style"]}>
            {p.market.currentOutcome.name} &nbsp;|&nbsp;{" "}
            {p.market.currentOutcome.stake - p.disputeBond} REP
          </p>
        </li>
        <li>
          <label>
          ...
          <ul className={FormStyles["Form__radio-buttons--per-line"]}>
            <li>
              <button
                className={classNames({
                  [`${FormStyles.active}`]: p.isMarketValid === true
                })}
                onClick={e => {
                  validateIsMarketValid(p.validations, p.updateState, true);
                }}
              >
                Yes
              </button>
            ...
            <li>
              <button
                className={classNames({
                  [`${FormStyles.active}`]: p.isMarketValid === false
                })}
                onClick={e => {
                  validateIsMarketValid(p.validations, p.updateState, false);
                }}
              >
                No
              </button>
        ...
        {p.isMarketValid &&
          (p.market.type === BINARY || p.market.type === CATEGORICAL) && (
            <li>
              <label>
                <span>Proposed Outcome</span>
              </label>
              <ul className={FormStyles["Form__radio-buttons--per-line"]}>
                {p.market.otherOutcomes &&
                  p.market.otherOutcomes.map(outcome => (
                    <li key={outcome.id}>
                      <button
                        className={classNames({
                          [`${FormStyles.active}`]:
                            p.selectedOutcome === outcome.name
                        })}
                        onClick={e => {
                          validateOutcome(
                            p.validations,
        ...
        {p.isMarketValid &&
          p.market.type === SCALAR && (
            <li className={FormStyles["field--short"]}>
              <label>
                <span htmlFor="sr__input--outcome-scalar">
                  Proposed Outcome
                </span>
                {p.validations.hasOwnProperty("selectedOutcome") &&
                  p.validations.selectedOutcome.length && (
        ...
        <li className={FormStyles["field--short"]}>
          <label>
            ...
            {p.validations.hasOwnProperty("stake") &&
              p.validations.stake.length && (
                <span className={FormStyles.Form__error}>
                  {p.validations.stake}
                </span>
              )}
          ...
          {p.stakeIsRequired &&
            p.minimumStakeRequired && (
              <p>
                A stake of greater than {p.minimumStakeRequired} REP is
                required.
              </p>
            )}
          <input
            ...
            value={p.stake}
            onChange={e => {
              validateStake(p, e.target.value);
            }}
          />
        </li>
      </ul>
