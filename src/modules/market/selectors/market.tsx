    ...
    assembledMarketsCache[marketID] = memoize(
      (
        ...
        const market = {
          ...
          formattedDescription: listWordsUnderLength(
        ...
        market.creationTime = formatDate(
          new Date(marketData.creationTime * 1000)
        );
        ...
        market.volume = formatShares(marketData.volume, {
          positiveSign: false
        });
        ...
        market.onSubmitPlaceTrade = outcomeID =>
          dispatch(
            placeTrade(marketID, outcomeID, marketTradeInProgress[outcomeID])
          );
        market.report = {
          ...marketReport,
          onSubmitReport: (
        ...
        market.outcomes = Object.keys(marketOutcomesData || {})
          .map(outcomeID => {
            ...
            const outcome = {
              ...
              id: outcomeID,
              marketID,
              lastPrice: formatEtherTokens(outcomeData.price || 0, {
                positiveSign: false
              })
            };
            if (market.isScalar) {
              ...
              if (outcome.lastPrice.value) {
                ...
                const midPoint = new BigNumber(market.minPrice, 10)
                  .plus(new BigNumber(market.maxPrice, 10))
                  .dividedBy(2);
          ...
          .sort(
            (a, b) =>
              b.lastPrice.value - a.lastPrice.value ||
              (a.name < b.name ? -1 : 1)
          );
        market.tags = (market.tags || []).filter(tag => !!tag);
        market.outstandingShares = formatNumber(
          getOutstandingShares(marketOutcomesData || {})
        );
        // TODO -- put back
        ...
        if (marketData.consensus) {
          ...
          if (market.type !== SCALAR && market.reportableOutcomes.length) {
            const marketOutcome = market.reportableOutcomes.find(
              outcome => outcome.id === market.consensus.outcomeID
            );
            if (marketOutcome)
          ...
          if (market.consensus.proportionCorrect) {
            market.consensus.percentCorrect = formatPercent(
              new BigNumber(market.consensus.proportionCorrect, 10).times(100)
            );
          }
        } else {
