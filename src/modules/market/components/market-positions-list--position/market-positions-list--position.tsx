...
export default class Position extends Component<PositionProps, PositionState> {
  static calcAvgDiff(position, order) {
    ...
    const orderPrice =
      getValue(order, "order.purchasePrice.formattedValue") || 0;
  ...
  render() {
    ...
    return (
      ...
      >
          ...
          {p.openOrders &&
            ...
            p.openOrders.map((order, i) => (
              <div key={i} className={Styles.Position__pending}>
                <span className={Styles["Position__pending-title"]}>
                  Pending
                </span>
                <span className={Styles["Position__pending-message"]}>
                  Bought {getValue(order, "order.qtyShares.formatted")} at{" "}
                  {getValue(order, "order.purchasePrice.formatted")} ETH
                </span>
              </div>
            ))}
        </li>
        ...
        {p.isExtendedDisplay &&
          !p.isMobile && <li>{getValue(p, "position.lastPrice.formatted")}</li>}
        {!p.isMobile && (
        ...
        {p.isExtendedDisplay && (
          <li>{getValue(p, "position.totalNet.formatted")}</li>
        )}
        <li>
          {getValue(p, "position.qtyShares.value") > 0 ? (
          ...
          {p.openOrders.length > 0 ? (
            ...
            <div className={Styles["Position__confirm-details"]}>
              <p>
                ...
                {getValue(p, "position.qtyShares.formatted")} shares of “{getValue(
                  p,
                  "name"
                )}” at {getValue(p, "position.avgPrice.formatted")} ETH?
              </p>
              <div className={Styles["Position__confirm-options"]}>
                <button
                  onClick={e => {
                    p.position.closePosition(
                      p.position.marketId,
                      p.position.outcomeId
                    );
                    this.toggleConfirm();
                  }}
                >
