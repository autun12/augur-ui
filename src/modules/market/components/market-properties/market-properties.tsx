...
const MarketProperties: React.SFC<MarketPropertiesProps> = p => {
  const shareVolumeRounded = setShareDenomination(
    getValue(p, "volume.rounded"),
  ...
  return (
    <article className={Styles.MarketProperties}>
      <ul className={Styles.MarketProperties__meta}>
        ...
        <li>
          <span>
            {dateHasPassed(p.endDate.timestamp) ? "Expired" : "Expires"}
          </span>
          <span>
            {p.isMobile ? p.endDate.formattedShort : p.endDate.formatted}
          </span>
        </li>
