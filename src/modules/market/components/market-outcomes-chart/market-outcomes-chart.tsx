...
type MarketOutcomesChartProps = {
  priceHistory: any[],
  selectedOutcomes?: any,
  updateSelectedOutcomes: (...args: any[]) => any
};
type MarketOutcomesChartState = {
  chart: any,
  chart: null,
  hoveredOutcome: null
};
export default class MarketOutcomesChart extends Component<
  MarketOutcomesChartProps,
  MarketOutcomesChartState
> {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidUpdate(prevProps) {
    if (!isEqual(prevProps.priceHistory, this.props.priceHistory))
      this.drawChart();
  }
  ...
  drawChart() {
    if (this.outcomesChart) {
      ...
      const xDomain = priceHistory.reduce(
        (p, outcome) => [...p, ...outcome.data.map(dataPoint => dataPoint[0])],
        []
      );
      const yDomain = priceHistory.reduce(
        (p, outcome) => [...p, ...outcome.data.map(dataPoint => dataPoint[1])],
        []
      );
      const xScale = d3
        .scaleTime()
