...
type MobilePositionsProps = {
  position: object,
  pendingOrders: any[]
};
type MobilePositionsState = {
  showConfirm: boolean,
  showConfirm: boolean
};
export default class MobilePositions extends Component<
  MobilePositionsProps,
  MobilePositionsState
> {
  static calcAvgDiff(position, orders) {
    const currentAvg =
      getValue(position, "position.avgPrice.formattedValue") || 0;
    const currentShares =
      getValue(position, "position.qtyShares.formattedValue") || 0;
  ...
  render() {
    ...
    return (
      <div className={CommonStyles.MarketPositionsListMobile__wrapper}>
        ...
        <div
          className={classNames(Styles.MobilePositions__confirm, {
          ...
          {p.pendingOrders.length > 0 ? (
            <div
              className={classNames(
                Styles["MobilePositions__confirm-details"],
                Styles.pending
              )}
            >
            ...
            <div className={Styles["MobilePositions__confirm-details"]}>
              ...
              <p>
                This will sell your{" "}
                {getValue(p, "position.position.qtyShares.formatted")} shares of
                &ldquo;{getValue(p, "position.name")}&rdquo; at market rate.
              </p>
              <div className={Styles["MobilePositions__confirm-options"]}>
                <button
                  onClick={e => this.setState({ showConfirm: !s.showConfirm })}
                ...
                <button
                  onClick={e => {
                    p.position.position.closePosition();
