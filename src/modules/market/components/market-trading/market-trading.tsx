...
class MarketTrading extends Component<MarketTradingProps, MarketTradingState> {
  constructor(props) {
    super(props);
    this.state = {
      showForm: false,
      ...
      if (nextProps.selectedOutcomes.length === 1) {
        this.setState({
          selectedOutcome: nextProps.market.outcomes.find(
            outcome => outcome.id === nextProps.selectedOutcomes[0]
          )
        });
      } else {
        this.setState({
      ...
      <section className={Styles.Trading}>
        ...
        {s.showOrderPlaced && (
          <div className={Styles["Trading__button--order-placed"]}>
            ...
            <button onClick={e => this.setState({ showOrderPlaced: false })}>
              {Close}
            </button>
          </div>
        )}
