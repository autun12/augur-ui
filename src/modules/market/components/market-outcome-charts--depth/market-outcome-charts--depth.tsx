...
type MarketOutcomeDepthProps = {
  ...
  updateHoveredDepth: (...args: any[]) => any
};
type MarketOutcomeDepthState = {
  yScale: any,
  xScale: any,
  chart: any,
  chartWidth: any,
...
> {
  ...
  componentDidUpdate(prevProps) {
    if (!isEqual(prevProps.marketDepth, this.props.marketDepth))
      this.drawChart();
  ...
  drawChart() {
    if (this.depthChart) {
      ...
      //  Fills
      const subtleGradient = chartDefs
      ...
      const xDomain = Object.keys(marketDepth).reduce(
        (p, side) => [
          ...p,
          ...marketDepth[side].reduce((p, item) => [...p, item[0]], [])
        ],
        []
      );
      ...
      const maxDiff = Math.abs(
        this.props.orderBookMid - this.props.orderBookMax
      );
      const minDiff = Math.abs(
        this.props.orderBookMid - this.props.orderBookMin
      );
      ...
      chart.append("text").attr("id", "hovered_price_label");
      // create crosshairs
  ...
  drawCrosshairs() {
    if (this.props.hoveredPrice == null) {
      ...
      const nearestCompletelyFillingOrder = Object.keys(
        this.props.marketDepth
      ).reduce((p, side) => {
        const fillingSideOrder = this.props.marketDepth[side].reduce(
          (p, order) => {
            if (p === null) return order;
            if (side === ASKS) {
              return this.props.hoveredPrice > p[1] &&
                this.props.hoveredPrice < order[1]
                ? order
                : p;
            }
            return this.props.hoveredPrice < p[1] &&
        ...
        return Math.abs(this.props.hoveredPrice - p[1]) <
          Math.abs(this.props.hoveredPrice - fillingSideOrder[1])
          ? p
          : fillingSideOrder;
      }, null);
