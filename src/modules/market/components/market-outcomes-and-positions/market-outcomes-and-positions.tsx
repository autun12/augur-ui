...
const MarketOutcomesAndPositions = p => (
  <section>
    ...
    {!p.isMobile && (
      <MarketPositionsList positions={p.positions} openOrders={p.openOrders} />
    )}
    {p.isMobile &&
      p.selectedOutcomes.length > 0 && (
        <MarketPositionsListMobile
          outcome={
            p.outcomes.filter(
              outcome => outcome.id === p.selectedOutcomes[0]
            )[0]
          }
          positions={p.positions.filter(
            position => position.id === p.selectedOutcomes[0]
          )}
          openOrders={p.openOrders.filter(
            order => order.id === p.selectedOutcomes[0]
          )}
          scalarShareDenomination={p.scalarShareDenomination}
        />
      )}
