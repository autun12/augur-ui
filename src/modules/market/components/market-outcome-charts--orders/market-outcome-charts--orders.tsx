...
type MarketOutcomeOrderbookProps = {
  orderBook: object,
...
type MarketOutcomeOrderbookState = {
  hoveredOrderIndex: any,
  hoveredSide: any,
...
export default class MarketOutcomeOrderbook extends Component<
  MarketOutcomeOrderbookProps,
  MarketOutcomeOrderbookState
> {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    ...
    const marketMidPoint = () => {
      ...
      if (p.orderBook.asks.length === 0 && p.orderBook.bids.length === 0) {
        ...
        midPoint =
          (p.orderBook.asks[p.orderBook.asks.length - 1].price +
            p.orderBook.bids[0].price) /
          2;
      }
          ...
          {(p.orderBook.asks || []).map((order, i) => (
            <div
              ...
              className={classNames(Styles.MarketOutcomeOrderBook__row, {
                [Styles["MarketOutcomeOrderBook__row--head"]]:
                  i === p.orderBook.asks.length - 1,
            ...
            >
              ...
              <div className={Styles.MarketOutcomeOrderBook__RowItem}>
                <span>
                  {order.cumulativeShares.toFixed(p.fixedPrecision).toString()}
                </span>
              </div>
            </div>
          ))}
