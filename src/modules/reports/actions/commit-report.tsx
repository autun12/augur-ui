...
export const updateReportCommitLock = (eventID, isLocked) => ({
...
export const commitReport = (
  ...
  console.log(
    `committing to report ${reportedOutcomeID} on market ${
      market.id
    } event ${eventID} period ${branch.reportPeriod}...`
  );
  const salt = bytesToHex(secureRandom(32));
  const fixedReport = augur.reporting.format.fixReport(
    reportedOutcomeID,
    market.minValue,
    market.maxValue,
    market.type,
  ...
  const report = {
    ...
    isIndeterminate,
    salt,
    reportHash: augur.reporting.crypto.makeHash(
      salt,
      fixedReport,
      eventID,
      loginAccount.address
