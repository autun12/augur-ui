...
> {
  ...
  handleOutcomeChange = e =>
    this.setState({ reportedOutcomeID: e.target.value });
  handleSubmit() {
    // TODO add amount to stake to form
    this.props.onClickSubmit(
      this.state.reportedOutcomeID,
  ...
  render() {
    ...
    return (
      <article
        className={classNames("report-form", { reported: s.isReported })}
      >
        <div className="outcome-options">
          <h4>{!s.isReported ? "Report the outcome" : "Outcome Reported"}</h4>
          {p.type === SCALAR && (
            <span>
              Enter the outcome of this event, if it was at least {p.minPrice}{" "}
              and at most {p.maxPrice}. If the outcome was above {p.maxPrice},
              you should report the outcome as {p.maxPrice}; if the outcome was
              below {p.minPrice}, you should report the outcome as {p.minPrice}.
            </span>
          )}
          <OutcomeOptions
            type={p.type}
        ...
        <div className="indeterminate">
          ...
          <span className="indeterminate-message">
            If this question is subjective, vague, or did not have a clear
            answer on the end date above, you should report indeterminate.
          </span>
          <Checkbox
            className={classNames("indeterminate-checkbox", {
        ...
        <div className="report-actions">
          {!s.isReported && (
            <button
              ...
              onClick={
                (!!s.reportedOutcomeID && !s.isReported && this.handleSubmit) ||
                undefined
              }
            >
              Submit Report
