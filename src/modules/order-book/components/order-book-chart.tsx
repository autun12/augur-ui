...
type OrderBookChartProps = {
  orderBookSeries?: object
};
export default class OrderBookChart extends Component<OrderBookChartProps, {}> {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    this.orderBookChart = new Highcharts.Chart("order_book_chart", {
      ...
      tooltip: {
        headerFormat: null,
        pointFormat:
          '<span style="color:{series.color}">{series.name}</span>: <b>{point.y} Shares @ {point.x} ETH Tokens</b><br/>',
        valueDecimals: 2
      },
      credits: {
        enabled: false
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.orderBookSeries !== this.props.orderBookSeries)
      this.updateChart();
  }
