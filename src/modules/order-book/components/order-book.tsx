...
type OrderBookProps = {
  ...
  selectedTradeSide: object,
  updateTradeFromSelectedOrder: (...args: any[]) => any,
  outcome?: object
};
type OrderBookState = {
  selectedNav: any
    ...
    return (
      <article className="order-book">
        {p.outcome && (
          <div>
            {p.marketType !== SCALAR ? (
              <h3>Order Book {name && <span>&mdash; {name}</span>}</h3>
            ) : (
              <h3>Order Book</h3>
            )}
            <ComponentNav
            ...
            {s.selectedNav === ORDER_BOOK_CHART && (
              <OrderBookChart orderBookSeries={orderBookSeries} />
            )}
          </div>
        )}
      </article>
