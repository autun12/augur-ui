  ...
  (address, positions, orderBooks) => {
    ...
    for (let i = 0; i < numAdjustedMarkets; ++i) {
      ...
      if (asks && Object.keys(asks).length) {
        positionsPlusAsks[marketID] = selectMarketPositionPlusAsks(
          address,
          positions[marketID],
          asks
        );
...
export const selectMarketPositionPlusAsks = (account, position, asks) => {
  ...
  if (asks) {
    ...
    for (let j = 0; j < numAdjustedOutcomes; ++j) {
      positionPlusAsks[adjustedOutcomes[j]] = new BigNumber(
        position[adjustedOutcomes[j]],
        10
      )
        .plus(getOpenAskShares(account, adjustedOutcomes[j], asks))
        .toFixed();
    }
  }
