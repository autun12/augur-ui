...
const ForkingProgressBar = p => {
  const forkingPeriodInDays =
    augur.constants.CONTRACT_INTERVAL.FORK_DURATION_SECONDS / SECONDS_PER_DAY;

  const daysRemaining = getDaysRemaining(p.forkEndTime, p.currentTime);

  const percentagePassed =
    (100 * (forkingPeriodInDays - daysRemaining)) / forkingPeriodInDays;
      ...
      <div className={Styles.ForkingProgressBar}>
        <div className={Styles.ForkingProgressBar__graph}>
          <div
            className={
              percentagePassed <= 90
                ? Styles.ForkingProgressBar__current
                : Styles.ForkingProgressBar__current_overflow
            }
          >
