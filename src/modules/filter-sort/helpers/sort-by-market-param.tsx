  ...
  return combinedFiltered.slice().sort((a, b) => {
    switch (selectedMarketParam) {
      ...
      case PARAMS.MARKET_END_DATE: {
        if (selectedSort) {
          return (
            getValue(items, `${b}.${selectedMarketParam}.timestamp`) -
            getValue(items, `${a}.${selectedMarketParam}.timestamp`)
          );
        }
        return (
          getValue(items, `${a}.${selectedMarketParam}.timestamp`) -
      ...
      case PARAMS.MARKET_MAKER_FEE: {
        if (selectedSort) {
          return (
            getValue(items, `${b}.${selectedMarketParam}.value`) -
            getValue(items, `${a}.${selectedMarketParam}.value`)
          );
        }
        return (
          getValue(items, `${a}.${selectedMarketParam}.value`) -
