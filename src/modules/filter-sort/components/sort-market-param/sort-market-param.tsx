...
type SortMarketParamState = {
  selectedMarketParam: any,
  selectedSort: boolean,
  selectedMarketParam: any,
  selectedSort: any
};
export default class SortMarketParam extends Component<
  SortMarketParamProps,
  SortMarketParamState
> {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    ...
    if (!selectedMarketParam || !selectedSort) {
      this.props.updateIndices({
        indices: sortByMarketParam(
          this.state.selectedMarketParam,
          this.state.selectedSort,
          this.props.items,
          this.props.combinedFiltered
        ),
    ...
    return (
      <article className={Styles.SortMarketParam}>
        ...
        >
          {s.selectedSort ? (
            <i
              className={classNames(Styles.fa, Styles["fa-sort-amount-desc"])}
            />
          ) : (
            <i
              className={classNames(Styles.fa, Styles["fa-sort-amount-asc"])}
            />
          )}
        </button>
        <Dropdown
          ...
          onChange={selectedMarketParam =>
            this.setState({ selectedMarketParam })
          }
        />
      </article>
    );
