...
type FilterSortControllerProps = {
  location: object,
  history: object,
  items: any[],
  updateFilteredItems: (...args: any[]) => any
};
type FilterSortControllerState = {
  combinedFiltered: any,
  [x: string]: any,
  children: any,
...
export default class FilterSortController extends Component<
  FilterSortControllerProps,
  FilterSortControllerState
> {
  constructor(props) {
    super(props);
  ...
  componentWillUpdate(nextProps, nextState) {
    const oldResults = Object.keys(this.state)
      .filter(key => key.indexOf("filter-sort-results--") !== -1)
      .reduce((p, key) => ({ ...p, [key]: this.state[key] }), {});
    const newResults = Object.keys(nextState)
      .filter(key => key.indexOf("filter-sort-results--") !== -1)
      .reduce((p, key) => ({ ...p, [key]: nextState[key] }), {});
    if (
  ...
  callFilterByCategory(query, items) {
    const category =
      query[CATEGORY_PARAM_NAME] != null
        ? decodeURIComponent(query[CATEGORY_PARAM_NAME])
        : null;
    this.updateIndices({
  ...
  callFilterByTags(query, items) {
    const tagsArray = parseStringToArray(
      decodeURIComponent(query[TAGS_PARAM_NAME] || ""),
  ...
  updateCombinedFilters(items, results) {
    const combinedFiltered = Object.keys(results).reduce((p, filterType) => {
      ...
      if (
        p.length === 0 ||
        (results[filterType] !== null && results[filterType].length === 0)
      )
        return [];
  ...
  updateSortedFiltered(rawSorted, combined) {
    this.props.updateFilteredItems(
      rawSorted !== null
        ? (rawSorted || []).filter(
            itemIndex => combined.indexOf(itemIndex) !== -1
          )
        : combined
    );
  }
  ...
  injectChildren(children, combinedFiltered) {
    ...
    const traverseChildren = child =>
      React.Children.map(child, subChild => {
        let subChildProps = {};
  ...
  render() {
    return <section>{this.state.children}</section>;
  }
}
