...
type FilterMarketStateProps = {
  location: object,
  history: object,
  items: any[],
  updateIndices: (...args: any[]) => any,
  currentReportingPeriod?: number
};
type FilterMarketStateState = {
  selectedMarketState: any,
  selectedMarketState: any
};
export default class FilterMarketState extends Component<
  FilterMarketStateProps,
  FilterMarketStateState
> {
  constructor(props) {
    super(props);
  ...
  componentWillMount() {
    const selectedMarketState = parseQuery(this.props.location.search)[
      FILTER_MARKET_STATE_PARAM
    ];
    if (selectedMarketState) {
      this.setState({ selectedMarketState });
    } else {
      this.props.updateIndices({
        indices: filterByMarketState(
          this.state.selectedMarketState,
          this.props.currentReportingPeriod,
          this.props.items
        ),
  ...
  componentWillUpdate(nextProps, nextState) {
    if (this.state.selectedMarketState !== nextState.selectedMarketState) {
      ...
      this.props.updateIndices({
        indices: filterByMarketState(
          nextState.selectedMarketState,
          nextProps.currentReportingPeriod,
          nextProps.items
        ),
        type: FILTER_MARKET_STATE_PARAM
      });
