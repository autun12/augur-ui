...
type PerformanceGraphState = {
  graphType: any,
  graphPeriod: any,
  graphType: string,
  graphTypeOptions: { label: string, value: string }[],
  graphTypeDefault: string,
  graphPeriod: string,
  graphPeriodOptions: { label: string, value: string }[],
  graphPeriodDefault: string
};
class PerformanceGraph extends Component<
  PerformanceGraphProps,
  PerformanceGraphState
> {
  constructor(props) {
    super(props);
    this.state = {
      ...
      yAxis: {
        ...
        tickPositioner() {
          ...
          let positions = [
            this.dataMin,
            this.dataMax / 2,
            Math.ceil(this.dataMax) + this.dataMax * 0.05
          ];
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      prevState.graphType !== this.state.graphType ||
      prevState.graphPeriod !== this.state.graphPeriod
    )
      this.updateChart();
  }
  ...
  updateChart() {
    (
      getValue(
        ...
        `${this.state.graphType}.${this.state.graphPeriod}`
      ) || []
    ).forEach((series, i) => {
