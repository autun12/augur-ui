...
> {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.newMarket.currentStep !== nextProps.newMarket.currentStep &&
  ...
  prevPage() {
    const newStep =
      this.props.newMarket.currentStep <= 0
        ? 0
        : this.props.newMarket.currentStep - 1;
    this.props.updateNewMarket({ currentStep: newStep });
  }
  nextPage() {
    const newStep =
      this.props.newMarket.currentStep >= this.state.pages.length - 1
              ...
              <div className={Styles.CreateMarketForm__navigation}>
                <button
                  className={classNames(Styles.CreateMarketForm__prev, {
                    [`${Styles["hide-button"]}`]: p.newMarket.currentStep === 0
                  })}
                ...
                {p.newMarket.currentStep < 4 && (
                  <button
                    className={classNames(Styles.CreateMarketForm__next, {
                      [`${Styles["hide-button"]}`]:
                        p.newMarket.currentStep === s.pages.length - 1
                    })}
