...
type CreateMarketResolutionProps = {
  newMarket: object,
...
type CreateMarketResolutionState = {
  date: any,
  date: any,
  focused: boolean
};
export default class CreateMarketResolution extends Component<
  CreateMarketResolutionProps,
  CreateMarketResolutionState
> {
  constructor(props) {
    super(props);
    this.state = {
      // expirySourceType: false,
      date: Object.keys(this.props.newMarket.endDate).length
        ? moment(this.props.newMarket.endDate.timestamp)
        : null,
      focused: false
    };
  ...
  validateDesignatedReporterAddress(value) {
    ...
    if (!isAddress(value)) {
      updatedMarket.validations[currentStep].designatedReporterAddress =
        "The Designated Reporter Address must be a valid Ethereum Address.";
    } else {
      ...
      <ul className={StylesForm.CreateMarketForm__fields}>
            ...
            >
              ...
              {p.newMarket.expirySourceType === EXPIRY_SOURCE_SPECIFIC && (
                <input
                  ...
                  onChange={e =>
                    p.validateField("expirySource", e.target.value)
                  }
                />
              )}
            </li>
          ...
          >
            <li>
              <button
                className={classNames({
                  [`${StylesForm.active}`]:
                    p.newMarket.designatedReporterType ===
                    DESIGNATED_REPORTER_SELF
                })}
                onClick={() =>
        ...
        <li className={Styles.CreateMarketResolution__datepicker}>
          ...
          <SingleDatePicker
            ...
            onDateChange={date => {
              this.setState({ date });
              p.validateField("endDate", formatDate(date.toDate()));
            }}
        ...
        <li>
          <label htmlFor="cm__input--time">
            ...
            {p.newMarket.validations[p.newMarket.currentStep].hour.length && (
              <span className={StylesForm.CreateMarketForm__error}>
                {InputErrorIcon}
                {p.newMarket.validations[p.newMarket.currentStep].hour}
              </span>
            )}
            {p.newMarket.validations[p.newMarket.currentStep].minute.length && (
              <span className={StylesForm.CreateMarketForm__error}>
                {InputErrorIcon}
                {p.newMarket.validations[p.newMarket.currentStep].minute}
              </span>
            )}
          </label>
          <div className={Styles.CreateMarketResolution__time}>
            <input
              ...
              placeholder="Hour"
              onChange={e =>
                p.validateNumber("hour", e.target.value, "hour", 1, 12, 0)
              }
            />
            <input
              type="number"
              ...
              onChange={e =>
                p.validateNumber("minute", e.target.value, "minute", 0, 59, 0)
              }
            />
            <InputDropdown
              label="AM/PM"
