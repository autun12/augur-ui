...
type CreateMarketPreviewProps = {
  newMarket: object
};
type CreateMarketPreviewState = {
  expirationDate: any,
  shares: string,
  expirationDate: any,
  shares: string
...
export default class CreateMarketPreview extends Component<
  CreateMarketPreviewProps,
  CreateMarketPreviewState
> {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      this.props.newMarket.endDate !== nextProps.newMarket.endDate ||
        ...
        <div className={Styles.CreateMarketPreview__header}>
          <div className={Styles["CreateMarketPreview__header-wrapper"]}>
            ...
            <h1 className={Styles.CreateMarketPreview__description}>
              {p.newMarket.description || "New Market Question"}
            </h1>
            <div className={Styles.CreateMarketPreview__outcome}>
              {(p.newMarket.type === BINARY || p.newMarket.type === SCALAR) && (
                <CreateMarketPreviewRange newMarket={p.newMarket} />
              )}
              {p.newMarket.type === CATEGORICAL &&
                p.newMarket.outcomes.length > 0 && (
                  <CreateMarketPreviewCategorical newMarket={p.newMarket} />
                )}
              {(p.newMarket.type === "" ||
                (p.newMarket.type === CATEGORICAL &&
                  p.newMarket.outcomes.length === 0)) &&
        ...
        <div className={Styles.CreateMarketPreview__footer}>
          <div className={Styles["CreateMarketPreview__footer-wrapper"]}>
            <ul className={Styles.CreateMarketPreview__meta}>
              ...
              <li>
                <span>
                  {dateHasPassed(p.newMarket.endDate.timestamp)
                    ? "Expired"
                    : "Expires"}
                </span>
