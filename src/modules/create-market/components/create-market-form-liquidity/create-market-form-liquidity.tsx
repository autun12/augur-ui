...
const PRECISION = 4;
type CreateMarketLiquidityProps = {
  newMarket: object,
  updateNewMarket: (...args: any[]) => any,
  validateNumber: (...args: any[]) => any,
  addOrderToNewMarket: (...args: any[]) => any,
  availableEth: string,
  isMobileSmall: boolean
};
type CreateMarketLiquidityState = {
  orderPrice: string,
  orderQuantity: string,
  minPrice: any,
  maxPrice: any,
  errors: { quantity: any[], price: any[] },
  isOrderValid: any,
  orderQuantity: any,
  orderPrice: any,
  orderEstimate: any,
  errors: { quantity: undefined[], price: undefined[] },
...
export default class CreateMarketLiquidity extends Component<
  CreateMarketLiquidityProps,
  CreateMarketLiquidityState
> {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentWillMount() {
    this.updatePriceBounds(
      this.props.newMarket.type,
      this.state.selectedOutcome,
      this.state.selectedNav,
      this.props.newMarket.orderBookSorted,
      this.props.newMarket.scalarSmallNum,
      this.props.newMarket.scalarBigNum
    );
  }
  ...
  componentWillUpdate(nextProps, nextState) {
    if (
      this.props.newMarket.type !== nextProps.newMarket.type ||
      this.props.newMarket.scalarSmallNum !==
  ...
  updateInitialLiquidityCosts(order, shouldReduce) {
    const minPrice =
      this.props.newMarket.type === SCALAR
        ? this.props.newMarket.scalarSmallNum
        : 0;
    const maxPrice =
      this.props.newMarket.type === SCALAR
    ...
    if (shouldReduce) {
      initialLiquidityEth = this.props.newMarket.initialLiquidityEth.minus(
        order.price.times(order.quantity)
      );
      initialLiquidityGas = this.props.newMarket.initialLiquidityGas.minus(
        new BigNumber(action.gasFees)
      );
      // initialLiquidityFees = this.props.newMarket.initialLiquidityFees.minus(new BigNumber(action.feeEth))
    } else {
      initialLiquidityEth = this.props.newMarket.initialLiquidityEth.plus(
        order.quantity.times(order.price)
      );
      initialLiquidityGas = this.props.newMarket.initialLiquidityGas.plus(
        new BigNumber(action.gasFees)
      );
  ...
  validateForm(orderQuantityRaw, orderPriceRaw) {
    ...
    if (
      ...
      orderPrice
        .times(orderQuantity)
        .plus(this.props.newMarket.initialLiquidityEth)
        .greaterThan(new BigNumber(this.props.availableEth))
    ) {
      ...
      orderQuantity !== "" &&
      orderQuantity.lessThanOrEqualTo(new BigNumber(0))
    ) {
      errors.quantity.push("Quantity must be positive");
    } else if (orderPrice !== "") {
      const bids = getValue(
        this.props.newMarket.orderBookSorted[this.state.selectedOutcome],
        `${BID}`
      );
      const asks = getValue(
        this.props.newMarket.orderBookSorted[this.state.selectedOutcome],
      ...
      if (this.props.newMarket.type !== SCALAR) {
        if (
          ...
          asks &&
          asks.length &&
          orderPrice.greaterThanOrEqualTo(asks[0].price)
        ) {
          errors.price.push(
          ...
          bids &&
          bids.length &&
          orderPrice.lessThanOrEqualTo(bids[0].price)
        ) {
          errors.price.push(
        ...
        errors.price.push(
          `Price cannot exceed ${this.state.maxPrice.toNumber()}`
        );
      } else if (orderPrice.lessThan(this.state.minPrice)) {
        errors.price.push(
          `Price cannot be below ${this.state.minPrice.toNumber()}`
        );
      }
    }
    if (
      ...
      <ul className={StylesForm.CreateMarketForm__fields}>
        <li className={Styles.CreateMarketLiquidity__settlement}>
          <label htmlFor="cm__input--settlement">
            ...
            {p.newMarket.validations[p.newMarket.currentStep].settlementFee
              .length && (
        ...
        <li className={Styles.CreateMarketLiquidity__order}>
          <div className={Styles["CreateMarketLiquidity__order-form"]}>
            <ul className={Styles["CreateMarketLiquidity__order-form-header"]}>
              ...
              >
                <button onClick={() => this.setState({ selectedNav: BID })}>
                  Buy
                </button>
              </li>
              <li
              ...
              >
                <button onClick={() => this.setState({ selectedNav: ASK })}>
                  Sell
                </button>
              </li>
            </ul>
            <ul className={Styles["CreateMarketLiquidity__order-form-body"]}>
              {p.newMarket.type === BINARY && (
                <li>
                  ...
                  <ul
                    className={classNames(
                      Styles["CreateMarketLiquidity__radio-buttons"],
                      StylesForm["CreateMarketForm__radio-buttons"]
                    )}
                  >
                    <li>
                      <button
                        className={classNames({
                          [`${StylesForm.active}`]: s.selectedOutcome === "Yes"
                        })}
                        onClick={() =>
                          this.setState({ selectedOutcome: "Yes" })
                        }
                      >
                        Yes
                      </button>
                    ...
                    <li>
                      <button
                        className={classNames({
                          [`${StylesForm.active}`]: s.selectedOutcome === "No"
                        })}
              ...
              {p.newMarket.type === CATEGORICAL && (
                <li>
                  ...
                  <InputDropdown
                    ...
                    default=""
                    options={p.newMarket.outcomes.filter(
                      outcome => outcome !== ""
                    )}
                    onChange={value =>
              ...
              <li>
                ...
                <input
                  ...
                  value={
                    s.orderQuantity instanceof BigNumber
              ...
              <li>
                ...
                <input
                  ...
                  value={
                    s.orderPrice instanceof BigNumber
