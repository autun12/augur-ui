...
type CreateMarketOutcomeState = {
  outcomeFieldCount: any,
  showAddOutcome: boolean,
  outcomeFieldCount: number,
  showAddOutcome: boolean,
  outcomeFieldCount: any,
  showAddOutcome: boolean,
  scalarMin: any,
...
export default class CreateMarketOutcome extends Component<
  CreateMarketOutcomeProps,
  CreateMarketOutcomeState
> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      scalarMin: speedomatic
        .unfix(speedomatic.constants.INT256_MIN_VALUE)
        .round(18, BigNumber.ROUND_DOWN),
      scalarMax: speedomatic
        .unfix(speedomatic.constants.INT256_MAX_VALUE)
        .round(18, BigNumber.ROUND_DOWN)
    };
  ...
  validateScalarNum(value, type) {
    ...
    const updatedMarket = { ...p.newMarket };
    let scalarSmallNum =
      type === "small" ? value : updatedMarket.scalarSmallNum;
      ...
      switch (true) {
        ...
        case scalarSmallNum.lessThan(this.state.scalarMin):
          updatedMarket.validations[
            currentStep
          ].scalarSmallNum = `Must be greater than: ${this.state.scalarMin.toNumber()}`;
          break;
        case scalarSmallNum.greaterThan(this.state.scalarMax):
          updatedMarket.validations[
            currentStep
          ].scalarSmallNum = `Must be less than: ${this.state.scalarMax.toNumber()}`;
          break;
        case scalarBigNum !== "" &&
          scalarSmallNum.greaterThanOrEqualTo(scalarBigNum):
          updatedMarket.validations[currentStep].scalarSmallNum =
            "Your minimum must be smaller than your maximum.";
          break;
        default:
    ...
    if (type === "big") {
      switch (true) {
        ...
        case scalarBigNum.lessThan(this.state.scalarMin):
          updatedMarket.validations[
            currentStep
          ].scalarBigNum = `Must be greater than: ${this.state.scalarMin.toNumber()}`;
          break;
        case scalarBigNum.greaterThan(this.state.scalarMax):
          updatedMarket.validations[
            currentStep
          ].scalarBigNum = `Must be less than: ${this.state.scalarMax.toNumber()}`;
          break;
        case scalarSmallNum !== "" &&
          scalarBigNum.lessThanOrEqualTo(scalarSmallNum):
          updatedMarket.validations[currentStep].scalarBigNum =
            "Your maximum must be larger than your minimum.";
          break;
        default:
    ...
    if (type === "tickSize") {
      if (value < 0) {
        updatedMarket.validations[currentStep].tickSize =
          "Tick size cannot be below zero.";
      } else {
  ...
  validateOutcomes(value, index) {
    ...
    switch (true) {
      ...
      case cleanedOutcomes.length > CATEGORICAL_OUTCOMES_MAX_NUM:
        ...
        break;
      case value !== "" &&
        cleanedOutcomes.filter(outcome => outcome === value).length >= 2:
        updatedMarket.validations[currentStep].outcomes =
          "Outcome name must be unique.";
        break;
      default:
  ...
  render() {
    const p = this.props;
    const s = this.state;
    const cleanedOutcomes = p.newMarket.outcomes.filter(
      outcome => outcome !== ""
    );
    return (
        ...
        <li>
          ...
          <ul className={StylesForm["CreateMarketForm__radio-buttons"]}>
            <li>
              <button
                className={classNames({
                  [`${StylesForm.active}`]: p.newMarket.type === BINARY
                })}
            ...
            <li>
              <button
                className={classNames({
                  [`${StylesForm.active}`]: p.newMarket.type === CATEGORICAL
                })}
            ...
            <li>
              <button
                className={classNames({
                  [`${StylesForm.active}`]: p.newMarket.type === SCALAR
                })}
          ...
          <li>
            ...
            <div className={Styles.CreateMarketOutcome__categorical}>
              {[...Array(s.outcomeFieldCount).keys()].map(i => {
                ...
                const isError =
                  typeof p.newMarket.validations[1].outcomes === "string";
                const isDuplicate =
                  cleanedOutcomes.filter(
                    outcome => outcome === p.newMarket.outcomes[i]
                  ).length >= 2;
                const needMinimum =
                  isError &&
                  i < 2 &&
                  (p.newMarket.outcomes[0] === "" ||
                    p.newMarket.outcomes[1] === "");
                const tooMany =
          ...
          <li>
            {p.newMarket.validations[p.newMarket.currentStep].scalarSmallNum
              ...
              <span className={StylesForm.CreateMarketForm__error}>
                {InputErrorIcon}
                {
                  p.newMarket.validations[p.newMarket.currentStep]
                    .scalarSmallNum
                }
              </span>
            )}
            ...
            {p.newMarket.validations[p.newMarket.currentStep].tickSize
              .length && (
              <span className={StylesForm["CreateMarketForm__error--field-50"]}>
                {InputErrorIcon}
                {p.newMarket.validations[p.newMarket.currentStep].tickSize}
            ...
            <div className={Styles.CreateMarketOutcome__scalar}>
              <div>
                ...
                <input
                  ...
                  max={s.scalarMax}
                  value={
                    p.newMarket.scalarSmallNum instanceof BigNumber
                      ? p.newMarket.scalarSmallNum.toNumber()
                      : p.newMarket.scalarSmallNum
                  }
              ...
              <div>
                ...
                <input
                  ...
                  max={s.scalarMax}
                  value={
                    p.newMarket.scalarBigNum instanceof BigNumber
                      ? p.newMarket.scalarBigNum.toNumber()
                      : p.newMarket.scalarBigNum
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    this.validateScalarNum(e.target.value, "tickSize")
                  }
                />
              </div>
            </div>
