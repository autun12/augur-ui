...
type AccountWithdrawProps = {
  isMobileSmall: boolean,
  eth: object,
  rep: object,
  transferFunds: (...args: any[]) => any
};
type AccountWithdrawState = { amount: any, isAmountValid: boolean } & {
  amount: any,
  isAmountValid: boolean
} & { address: any, isAddressValid: boolean } & {
  address: any,
  isAddressValid: boolean
} & { isValid: boolean } & { isValid: boolean } & any & {
    selectedAsset: any,
    upperBound: any
  } & any;
...
> {
  ...
  validateAmount(amount) {
    ...
    if (
      isNaN(parseFloat(sanitizedAmount)) ||
      !isFinite(sanitizedAmount) ||
      (sanitizedAmount > this.state.upperBound || sanitizedAmount <= 0)
    ) {
      this.setState({
  ...
  submitForm() {
    ...
    if (s.isValid) {
      const stringedAmount =
        s.amount instanceof BigNumber ? s.amount.toString() : s.amount;
      ...
      <section className={Styles.AccountWithdraw}>
        ...
        <div className={Styles.AccountWithdraw__main}>
          <div className={Styles.AccountWithdraw__description}>
            <p>
              Withdraw Ethereum or Reputation from another account into your
              Trading Account connected with Augur.
            </p>
                ...
                <label htmlFor="currency">
                  ...
                  <InputDropdown
                    ...
                    onChange={type => {
                      ...
                      const upperBound =
                        type === "ETH" ? p.eth.value : p.rep.value;
                      this.setState({
                        selectedAsset,
                        upperBound
                      });
