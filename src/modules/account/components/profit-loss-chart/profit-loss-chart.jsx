...
export default class ProfitLossChart extends Component {
  ...
  componentDidMount() {
    ...
    this.profitLossChart = new Highcharts.Chart(chartId, {
      ...
      tooltip: {
        pointFormat:
          '<span style="color: #372e4b;">{series.name}</span>: <b>{point.y} ETH Tokens</b><br/>',
        valueDecimals: 2
      },
      credits: {
        enabled: false
  ...
  updateChart() {
    const { id, isMobile, series } = this.props;
    ...
    if (
      this.refs[containerId] &&
      this.refs[containerId].clientWidth <= height
    ) {
  ...
  render() {
    const { id, title, totalValue } = this.props;
    const containerId = "profit_loss_chart_container" + id;
    const chartId = "profit_loss_chart" + id;

    return (
      <article className={Styles.ProfitLossChart} ref={containerId}>
        <div id={chartId} />
        <div className={Styles.ProfitLossChart__Title}>
          <span className={Styles["ProfitLossChart__Title-label"]}>
            {title + " "}
          </span>
          <span className={Styles["ProfitLossChart__Title-value"]}>
            {totalValue}
          </span>
        </div>
      </article>
