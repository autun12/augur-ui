...
type SideNavProps = {
  defaultMobileClick: (...args: any[]) => any,
  isMobile: boolean,
  isLogged?: boolean,
  menuData: any[],
  mobileShow: boolean,
...
export default class SideNav extends Component<SideNavProps, SideNavState> {
  ...
  render() {
    const { isMobile, isLogged, toggleNotifications, unseenCount } = this.props;
    const accessFilteredMenu = this.props.menuData.filter(
      item => !(item.requireLogin && !isLogged)
    );
    return (
      <aside
        className={classNames(Styles.SideNav, {
          [`${Styles.mobileShow}`]: this.props.mobileShow
        })}
      >
