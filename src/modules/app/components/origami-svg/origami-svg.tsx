...
export default class Origami extends Component<OrigamiProps, {}> {
  componentWillMount() {
  ...
  getDesktopShapes() {
    ...
    const applySecondaryOffset = point =>
      offsetPoint(point, [secondaryXOffset, 0]);
    ...
    const shapes = [
      ...
      {
        color: "#9592A4",
        points: [this.plgram.tl, this.plgram.bl, this.plgram.br].map(
          applyMainOffset
        )
      },
      {
        color: "#C5C3CD",
        points: [this.plgram.tl, this.plgram.tr, this.plgram.br].map(
          applyMainOffset
        )
      }
    ];
  ...
  getMobileShapes() {
    const shapes = [
      ...
      {
        color: "#553580",
        points: [[147, 0], this.mobilePlgram.tl, this.mobilePlgram.tr, [257, 0]]
      },
      {
        color: "#9592A4",
        points: [
