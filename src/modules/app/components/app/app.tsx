...
type AppViewProps = {
  ...
  updateIsMobile: (...args: any[]) => any,
  updateIsMobileSmall: (...args: any[]) => any,
  initAugur: (...args: any[]) => any,
  modal: object,
...
type AppViewState = {
  ...
  mobileMenuState: number,
  mobileMenuState: number,
  mainMenu: { scalar: number, open: boolean, currentTween: null },
  subMenu: { scalar: number, open: boolean, currentTween: null },
  mobileMenuState: number,
...
export default class AppView extends Component<AppViewProps, AppViewState> {
  ...
  changeMenu(nextBasePath) {
    ...
    if ((newType === AccountInnerNav && !p.isLogged) || oldType === newType) {
      return;
    }
    const openNewMenu = () => {
  ...
  render() {
    ...
    if (!p.isMobile) {
      if (parsePath(p.location.pathname)[0] === AUTHENTICATION) {
    ...
    return (
      <main>
        ...
        />
        {Object.keys(p.modal).length !== 0 && <Modal />}
        <div
        ...
        >
          <section className={Styles.SideBar}>
            <Origami isMobile={p.isMobile} menuScalar={origamiScalar} />
            ...
            >
              {InnerNav && (
                <InnerNav
                  ...
                  openSubMenu={() =>
                    this.setState({
                      mobileMenuState: mobileMenuStates.SUBMENU_OPEN
                    })
                  }
