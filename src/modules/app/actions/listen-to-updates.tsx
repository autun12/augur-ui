        ...
        WinningTokensRedeemed: (err, log) => {
          ...
          if (log) {
            ...
            if (log.reporter === getState().loginAccount.address) {
              dispatch(updateAssets());
              dispatch(
                convertLogsToTransactions(TYPES.REDEEM_WINNING_TOKENS, [log])
              );
            }
          }
        },
              ...
              dispatch(
                loadMarketsInfo([log.marketID], () => {
                  const {
                    ...
                    author,
                    description
                  } = getState().marketsData[log.marketID];
                  dispatch(
                    updateMarketCategoryPopularity(
                      log.marketID,
                      new BigNumber(volume, 10).neg().toNumber()
                    )
                  );
