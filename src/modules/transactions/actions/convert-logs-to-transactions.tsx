        ...
        augur.markets.getMarketsInfo(
          ...
          (err, marketsInfo) => {
            if (
              !marketsInfo ||
              marketsInfo.error ||
              !Array.isArray(marketsInfo) ||
              !marketsInfo.length ||
              !marketsInfo[0]
            ) {
              if (marketsInfo && marketsInfo.error)
                console.error("augur.markets.getMarketsInfo:", marketsInfo);
              return next(
                `[${label}] couldn't load market info for market ${marketID}: ${JSON.stringify(
                  data
                )}`
              );
            }
...
export const convertLogToTransaction = (
  label,
  log,
  status,
  isRetry,
  ...
  if (hash) {
    ...
    const gasFees =
      transactionData && transactionData.gasFees
        ? transactionData.gasFees.value
        : null;
    if (log.removed) {
      // TODO rollback
    ...
    const transaction = dispatch(
      constructTransaction(label, log, isRetry, callback)
    );
    if (transaction) {
      dispatch(
