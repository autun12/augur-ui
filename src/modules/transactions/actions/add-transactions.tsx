  ...
  return (dispatch, getState) => {
    dispatch(
      updateTransactionsData(
        transactionsArray.reduce((p, transaction) => {
...
function buildTradeTransaction(trade, marketsData) {
  ...
  meta["gas fees"] = transaction.hasOwnProperty("gasFees")
    ? transaction.gasFees
    : 0;
  ...
  transaction.message = `${transaction.type} ${transaction.amount} Shares @ ${
    transaction.price
  } ETH`;
  ...
  return (dispatch, getState) => {
    ...
    each(transfers, transfer => {
      ...
      meta["gas fees"] = transaction.hasOwnProperty("gasFees")
        ...
        : 0;
      meta.confirmations =
        blockchain.currentBlockNumber - transaction.creationBlockNumber;
      transaction.meta = meta;
      header.message = "Transfer";
      header.description = `${transaction.value} ${
        transaction.symbol
...
export function addMarketCreationTransactions(marketsCreated) {
  return (dispatch, getState) => {
    ...
    each(marketsCreated, marketID => {
      ...
      meta["creation fee"] =
        transaction.hasOwnProperty("creationFee") &&
        transaction.creationFee !== undefined
      ...
      eachOf(value, (value2, index) => {
        eachOf(value2, (value3, type) => {
          eachOf(value3, (value4, outcomeID) => {
            ...
            transaction.id = transaction.transactionHash + transaction.logIndex;
            transaction.message = `${transaction.orderState} - ${type} ${
              transaction.amount
    ...
    eachOf(reports, (value, universe) => {
      eachOf(value, (value1, marketID) => {
        eachOf(value1, (report, index) => {
          ...
          header.description = `Staked  ${
            transaction.amountStaked
          } REP on market ${market.description}`;
          // create unique id
          transactions[transaction.id] = header;
        });
      });
...
function buildHeader(item, type, status) {
  ...
  header.timestamp = convertUnix(
    item.timestamp ? item.timestamp : item.creationTime
  );
