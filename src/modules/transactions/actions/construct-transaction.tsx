...
export function loadDataForMarketTransaction(label, log, isRetry, callback) {
  return (dispatch, getState) => {
    ...
    if (!market || !market.description) {
      ...
      return dispatch(
        loadMarketThenRetryConversion(marketID, label, log, callback)
      );
    }
    return market;
  };
...
export function constructCollectedFeesTransaction(log) {
  ...
  const initialRepBalance =
    log.initialRepBalance !== undefined
      ? log.initialRepBalance
      : new BigNumber(log.newRepBalance).minus(repGain).toFixed();
  ...
  if (log.totalReportingRep) {
    ...
    if (!totalReportingRep.eq(ZERO)) {
      const percentRep = formatPercent(
        new BigNumber(initialRepBalance, 10)
          .dividedBy(totalReportingRep)
          .times(100),
        { decimals: 0 }
      );
  ...
  if (log.cashFeesCollected !== undefined && log.repGain !== undefined) {
    transaction.data.balances = [
      {
        change: formatEtherTokens(log.cashFeesCollected, {
          positiveSign: true
        }),
...
export function constructTradingProceedsClaimedTransaction(
  log,
  market,
  dispatch
) {
...
export function constructSubmitReportTransaction(
  log,
  marketID,
  market,
  outcomes,
  ...
  const formattedReport = formatReportedOutcome(
    log.report,
    market.minPrice,
    market.maxPrice,
    market.type,
  ...
  transaction.message = `${action} report: ${formatReportedOutcome(
    log.report,
    market.minPrice,
    market.maxPrice,
    market.type,
  ...
  if (!log.inProgress) {
    dispatch(
      updateMarketsWithAccountReportData({
        [marketID]: {
...
export function constructTransferTransaction(log, address) {
  ...
  if (log._from === address) {
    ...
    transaction.data.balances = [
      {
        change: formatRep(new BigNumber(log._value, 10).neg(), {
          positiveSign: true
        })
      }
    ];
...
export const constructCancelOrderTransaction = (
  trade,
  marketID,
  marketType,
  description,
  ...
  const displayPrice = augur.trading.denormalizePrice({
    normalizedPrice: trade.price,
    minPrice,
    maxPrice
  });
  ...
  return {
    [trade.transactionHash]: {
      ...
      message: `${action} order to ${trade.orderType} ${
        formattedShares.full
      } for ${formattedPrice.full} each`,
      ...
      totalReturn: trade.inProgress
        ? null
        : formatEtherTokens(trade.cashRefund),
      gasFees:
        trade.gasFees && new BigNumber(trade.gasFees, 10).gt(ZERO)
          ? formatEther(trade.gasFees)
          : null,
...
export const constructCreateOrderTransaction = (
  trade,
  marketID,
  marketType,
  description,
  ...
  return {
    [trade.transactionHash]: {
      ...
      message: `${action} ${formattedShares.full} for ${
        formatEtherTokens(
          unfix(
            trade.orderType === TYPES.BUY
              ? fxpTotalCostPerShare
              : fxpTotalReturnPerShare
          )
        ).full
      } / share`,
      numShares: formattedShares,
      ...
      freeze: {
        ...
        noFeeCost:
          orderType === TYPES.SELL
            ? undefined
            : formatEtherTokens(unfix(fxpNoFeeCost)),
        settlementFee: formatEtherTokens(settlementFee)
      },
      avgPrice: formattedPrice,
      timestamp: formatDate(new Date(trade.timestamp * 1000)),
      hash: trade.transactionHash,
      feePercent: formatPercent(
        unfix(
          fxpSettlementFee
            .dividedBy(fxpTotalCost)
            .times(TEN_TO_THE_EIGHTEENTH_POWER)
            .floor()
        ).times(100)
      ),
      totalCost:
        orderType === TYPES.BUY
          ? formatEtherTokens(unfix(fxpTotalCost))
          : undefined,
      totalReturn:
        orderType === TYPES.SELL
          ? formatEtherTokens(unfix(fxpTotalReturn))
          : undefined,
      gasFees:
        trade.gasFees && new BigNumber(trade.gasFees, 10).gt(ZERO)
          ? formatEther(trade.gasFees)
...
export const constructFillOrderTransaction = (
  trade,
  marketID,
  marketType,
  description,
  ...
  if (trade.isMaker) {
    orderType =
      trade.orderType === TYPES.SELL ? TYPES.MATCH_BID : TYPES.MATCH_ASK;
    perfectOrderType = trade.orderType === TYPES.SELL ? "bought" : "sold";
    formattedTotalCost =
      trade.orderType === TYPES.SELL
        ? formatEtherTokens(bnTotalCost)
        : undefined;
    formattedTotalReturn =
      trade.orderType === TYPES.BUY
        ? formatEtherTokens(bnTotalReturn)
        : undefined;
  } else {
  ...
  const transaction = {
    [transactionID]: {
      ...
      settlementFee: formatEtherTokens(settlementFee),
      feePercent: formatPercent(
        bnSettlementFee.dividedBy(bnTotalCost).times(100)
      ),
...
export const constructTradingTransaction = (
  label,
  trade,
  marketID,
  outcomeID,
  ...
  if (marketType === BINARY || marketType === SCALAR) {
    ...
    outcomeName = (marketOutcomesData ? marketOutcomesData[outcomeID] : {})
      .name;
  }
  switch (label) {
    case TYPES.CANCEL_ORDER: {
      return dispatch(
        constructCancelOrderTransaction(
          trade,
          marketID,
          marketType,
          description,
    ...
    case TYPES.CREATE_ORDER: {
      return dispatch(
        constructCreateOrderTransaction(
          trade,
          marketID,
          marketType,
          description,
...
export const constructTransaction = (label, log, isRetry, callback) => (
  dispatch,
  getState
) => {
  switch (label) {
    ...
    case TYPES.CREATE_MARKET: {
      const market = dispatch(
        loadDataForMarketTransaction(label, log, isRetry, callback)
      );
      if (!market || !market.description) break;
      return constructCreateMarketTransaction(
        log,
        market.description,
        dispatch
      );
    ...
    case TYPES.SUBMIT_REPORT: {
      const aux = dispatch(
        loadDataForReportingTransaction(label, log, isRetry, callback)
      );
      if (!aux) break;
      return constructSubmitReportTransaction(
        log,
        aux.marketID,
        aux.market,
        aux.outcomes,
