...
export const generateTrade = memoize(
  (market, outcome, outcomeTradeInProgress, orderBooks) => {
    ...
    const gasFeesRealEth =
      (outcomeTradeInProgress && outcomeTradeInProgress.gasFeesRealEth) || 0;
    const totalCost =
    ...
    return {
      ...
      potentialEthProfit: preOrderProfitLoss
        ? formatEtherTokens(preOrderProfitLoss.potentialEthProfit)
        : null,
      potentialEthLoss: preOrderProfitLoss
        ? formatEtherTokens(preOrderProfitLoss.potentialEthLoss)
        : null,
  ...
  tradeOrders => {
    ...
    if (tradeOrders && tradeOrders.length) {
      tradeSummary = tradeOrders.reduce((p, tradeOrder) => {
        ...
        if (
          tradeOrder.data &&
          tradeOrder.data.gasFees &&
          tradeOrder.data.gasFees.value
        ) {
          p.totalGas = p.totalGas.plus(
            new BigNumber(tradeOrder.data.gasFees.value, 10)
          );
        }
        // trade order
...
export const generateTradeOrders = memoize(
  (market, outcome, outcomeTradeInProgress) => {
    ...
    return tradeActions.map(tradeAction => {
      ...
      const avgPrice = new BigNumber(costEth, 10).dividedBy(
        new BigNumber(numShares, 10)
      );
      const noFeePrice =
        market.type === "scalar"
