...
export function updateTradesInProgress(
  ...
  return (dispatch, getState) => {
    ...
    if (
      (limitPrice === "" ||
        (parseFloat(limitPrice) === 0 && market.type !== SCALAR)) &&
      numShares === null
    ) {
      return dispatch({
        type: UPDATE_TRADE_IN_PROGRESS,
    ...
    if (
      cleanNumShares &&
      !cleanLimitPrice &&
      (market.type === SCALAR || cleanLimitPrice !== "0")
    ) {
    ...
    if (
      ...
      dispatch(
        loadAccountPositions({ market: marketID }, (err, accountPositions) => {
          if (err) {
            return dispatch({
              type: UPDATE_TRADE_IN_PROGRESS,
          ...
          const simulatedTrade = augur.trading.simulateTrade({
            ...
            tokenBalance:
              (loginAccount.ethTokens && loginAccount.ethTokens.toString()) ||
              "0",
          ...
          const totalFee = new BigNumber(
            simulatedTrade.settlementFees,
            10
          ).plus(new BigNumber(simulatedTrade.gasFees, 10));
          newTradeDetails.totalFee = totalFee.toFixed();
          newTradeDetails.totalCost = new BigNumber(
            simulatedTrade.tokensDepleted,
            10
          )
            .neg()
            .toFixed();
          newTradeDetails.feePercent = totalFee
            .dividedBy(new BigNumber(simulatedTrade.tokensDepleted, 10))
            .toFixed();
          ...
          dispatch({
            ...
            data: {
              marketID,
              outcomeID,
              details: { ...newTradeDetails, ...simulatedTrade }
            }
          });
