...
export default class Help extends Component<{}, HelpState> {
  ...
  toggleDefinition(index) {
    this.setState({
      visibleDefinitions:
        this.state.visibleDefinitions.indexOf(index) !== -1
    ...
    return (
      <div className={Styles.Help}>
        <button
          ...
          onClick={() =>
            toggleHeight(this.helpItems, s.areQuestionsVisible, () =>
              this.setState({ areQuestionsVisible: !s.areQuestionsVisible })
            )
          }
        >
        ...
        >
          {HELPS.map((help, i) => (
            <div key={help.title} className={Styles.Help__item}>
              <button
                onClick={() =>
                  toggleHeight(
                    this.helpItem[i],
                    this.state.visibleDefinitions.indexOf(i) !== -1,
                    () => this.toggleDefinition(i)
                  )
                }
              >
                {help.title}{" "}
