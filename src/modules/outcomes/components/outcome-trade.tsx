...
type OutcomeTradeProps = {
  selectedShareDenomination?: string,
  updateSelectedTradeSide?: (...args: any[]) => any,
  marketType?: string,
  minLimitPrice?: string,
  maxLimitPrice?: string,
  submitTrade?: (...args: any[]) => any,
  tradeSummary?: object
};
type OutcomeTradeState = {
  minLimitPrice: any,
  maxLimitPrice: any,
  shareInputPlaceholder: string,
  sharesDenominated: any,
  timestamp: number,
  isLimitPriceValueValid: boolean,
  isLimitPriceValueValid: boolean,
  isSharesValueValid: boolean,
  isSharesValueValid: boolean,
  timestamp: number,
  shareInputPlaceholder: string,
  sharesDenominated: any,
...
> {
  constructor(props) {
    ...
    this.state = {
      timestamp: Date.now(),
      shareInputPlaceholder: generateShareInputPlaceholder(
        props.selectedShareDenomination
      ),
      ...
      sharesDenominated:
        denominateShares(
          getValue(props, "selectedOutcome.trade.numShares"),
          SHARE,
          props.selectedShareDenomination
        ) || "",
      minLimitPrice:
        props.marketType && props.marketType === SCALAR
          ? props.minLimitPrice
          : 0,
      maxLimitPrice:
        props.marketType && props.marketType === SCALAR
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.marketType !== nextProps.marketType) {
      this.setState({
        minLimitPrice:
          nextProps.marketType && nextProps.marketType === SCALAR
            ? nextProps.minLimitPrice
            : 0,
        maxLimitPrice:
          nextProps.marketType && nextProps.marketType === SCALAR
            ? nextProps.maxLimitPrice
            : 1
      });
    }
    const oldTrade = JSON.stringify(
      getValue(this.props, "selectedOutcome.trade")
    );
    const newTrade = JSON.stringify(
      getValue(nextProps, "selectedOutcome.trade")
    );
    if (
      newTrade !== oldTrade ||
      this.props.selectedShareDenomination !==
        nextProps.selectedShareDenomination
    ) {
      this.setState({
        shareInputPlaceholder: generateShareInputPlaceholder(
          nextProps.selectedShareDenomination
        ...
        sharesDenominated:
          denominateShares(
            getValue(nextProps, "selectedOutcome.trade.numShares"),
            SHARE,
            nextProps.selectedShareDenomination
          ) || ""
      });
    ...
    if (
      oldTradeOrders.length !== newTradeOrders.length &&
      newTradeOrders.length === 0
    ) {
      this.updateTimestamp();
    }
  }
  ...
  updateSelectedNav(selectedTradeSide, id) {
    ...
    const orderBookSide = getValue(
      this.props,
      `selectedOutcome.orderBook.${availableOrderType}`
    );
  ...
  handleSharesInput(value) {
    const trade = getValue(this.props, "selectedOutcome.trade");
    const valueDenominated = denominateShares(
      value,
      this.props.selectedShareDenomination,
      SHARE
    );
  ...
  validatePrice(value, trade) {
    if (value != null) {
      if (
        (value >= parseFloat(this.state.minLimitPrice) &&
          value <= parseFloat(this.state.maxLimitPrice)) ||
        value === ""
      ) {
  ...
  render() {
    ...
    const selectedTradeSide =
      (selectedID && p.selectedTradeSide[selectedID]) || BUY;
    const tradeOrder = getValue(p, "tradeSummary.tradeOrders").find(
      order => order.data.outcomeID === selectedID
    );
    const hasFunds = getValue(p, "tradeSummary.hasUserEnoughFunds");
    return (
      <article className="outcome-trade market-content-scrollable">
        {p.marketType !== SCALAR ? (
          <h3>Create Order {name && <span>&mdash; {name}</span>}</h3>
        ) : (
          <h3>Create Order</h3>
        )}
        {trade && (
          <div key={s.timestamp} className="outcome-trade-inputs">
            <div className="outcome-trade-inputs-sides">
              <ComponentNav
                fullWidth
                navItems={p.outcomeTradeNavItems}
                selectedNav={selectedTradeSide}
                updateSelectedNav={side => {
                  this.updateSelectedNav(side, selectedID);
                }}
              />
            </div>
            <div className="outcome-trade-inputs-fields">
              ...
              <span>@</span>
              <Input
                className={classNames("trade-price-input", {
                  "input-error": !s.isLimitPriceValueValid
                })}
                placeholder="Price"
        ...
        {!s.isLimitPriceValueValid && (
          <span className="outcome-trade-input-error-message">
            {`Limit price must be between ${s.minLimitPrice} - ${
              s.maxLimitPrice
            }.`}
          </span>
        )}
        {tradeOrder &&
          s.isSharesValueValid &&
          s.isLimitPriceValueValid && (
            <OutcomeTradeSummary trade={trade} tradeOrder={tradeOrder} />
          )}
        {tradeOrder &&
          s.isSharesValueValid &&
          s.isLimitPriceValueValid && (
