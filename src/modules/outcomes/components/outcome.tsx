...
const Outcome = p => {
  ...
  const topBidShares = setShareDenomination(
    getValue(p, "outcome.topBid.shares.formatted"),
    p.selectedShareDenomination
  );
  const topAskShares = setShareDenomination(
    getValue(p, "outcome.topAsk.shares.formatted"),
    p.selectedShareDenomination
  );
  ...
  return (
    <article
      className={classNames("outcome", {
        selected: selectedOutcomeID === p.outcome.id
      })}
    >
      <a
        className={classNames("unstlyed outcome-row-full", {
          selected: selectedOutcomeID === p.outcome.id
        })}
        onClick={() => {
      ...
      <a
        className={classNames("unstlyed outcome-row-condensed", {
          selected: selectedOutcomeID === p.outcome.id
        })}
        onClick={() => {
          p.updateSelectedOutcome(p.outcome);
      ...
      >
        {p.marketType === SCALAR ? (
          ...
          <span className="outcome-summary">
            ...
            <span>
              Last Price: <ValueDenomination formatted={lastPrice} />
            </span>
          </span>
        )}
